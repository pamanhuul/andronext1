...
class SetStatusDialogFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.clearStatus.setTextColor(ThemeUtils.primaryColor(context, true))
        ThemeUtils.colorPrimaryButton(binding.setStatus, context)
        binding.customStatusInput.background.colorFilter =
            PorterDuffColorFilter(ThemeUtils.primaryAccentColor(context), PorterDuff.Mode.SRC_ATOP)
        binding.customStatusInput.highlightColor = ThemeUtils.primaryColor(activity)
    }
    ...
    private fun visualizeStatus(statusType: StatusType) {
        when (statusType) {
            StatusType.ONLINE -> {
                clearTopStatus()
                binding.onlineStatus.setBackgroundColor(ThemeUtils.primaryColor(context))
            }
            StatusType.AWAY -> {
                clearTopStatus()
                binding.awayStatus.setBackgroundColor(ThemeUtils.primaryColor(context))
            }
            StatusType.DND -> {
                clearTopStatus()
                binding.dndStatus.setBackgroundColor(ThemeUtils.primaryColor(context))
            }
            StatusType.INVISIBLE -> {
                clearTopStatus()
                binding.invisibleStatus.setBackgroundColor(ThemeUtils.primaryColor(context))
            }
            else -> clearTopStatus()
        }
    }
